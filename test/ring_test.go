...
func verify(t *testing.T, r *Thing2Ring, N int, sum int) {
	// Len
	n := r.Len()
	if n != N {
	...
	if float64(sum) >= 0 && s != float64(sum) {
	...
	// connections
	if r.next != nil {
		var p *Thing2Ring // previous element
...
func TestLink2(t *testing.T) {
	var r0 *Thing2Ring
	r1a := &Thing2Ring{Value: 42}
	r1b := &Thing2Ring{Value: 77}
	r10 := makeN(10)
...
func TestLink3(t *testing.T) {
	...
	for i := 1; i < 100; i++ {
		n += i
		verify(t, r.Link(NewThing2Ring(i)), n, -1)
	}
}
