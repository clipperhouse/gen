...
func verify(t *testing.T, r *ThingRing, N int, sum int) {
	// Len
	n := r.Len()
	if n != N {
	...
	// connections
	if r.next != nil {
		var p *ThingRing // previous element
...
func TestLink2(t *testing.T) {
	var r0 *ThingRing
	r1a := &ThingRing{Value: 42}
	r1b := &ThingRing{Value: 77}
	r10 := makeN(10)
...
func TestLink3(t *testing.T) {
	...
	for i := 1; i < 100; i++ {
		n += i
		verify(t, r.Link(NewThingRing(i)), n, -1)
	}
}
